...
{
	os << "year: " << y.year << '\n';
	int max_month = static_cast<int>(y.month.size());
	for (int m = 1; m < max_month; ++m)
	{
		const Month each_month = y.month[m];
		if (each_month.month != not_a_month)
		{
			os << "month: " << m << '\n';
			int max_day = static_cast<int>(each_month.day.size());
			for (int d = 1; d < max_day; ++d)
			{
				const Day each_day = each_month.day[d];
				const int max_hour = static_cast<int>(each_day.hour.size());
				for (int h = 0; h < max_hour; ++h)
				{
					const double temperature = each_day.hour[h];
					if (temperature != not_a_reading)
						os << "day: " << d << '\t' << "hour: " << h << '\t' << "temperature: " << temperature << '\n';
				}
			}
		}
	}
...
//input file name：exercises/Ex_20_readings.txt
//output file name：exercises/Ex_20_output.txt
void Ex_20()
{
	read_format();
}